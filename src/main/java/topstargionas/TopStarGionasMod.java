...
import topstargionas.relics.BaseRelic;
import topstargionas.util.GeneralUtils;
import topstargionas.util.KeywordInfo;
import topstargionas.util.TextureLoader;
...
@SpireInitializer
public class TopStarGionasMod implements
    ...
    public static void initialize() {
        new TopStarGionasMod();

        TheCaretaker.Meta.registerColor();
    }

    public TopStarGionasMod() {
    ...
    private static String checkResourcesPath() {
        String name = TopStarGionasMod.class.getName(); //getPackage can be iffy with patching, so class name is used instead.
        ...
        if (!resources.exists()) {
            throw new RuntimeException("\n\tFailed to find resources folder; expected it to be at  \"resources/" + name + "\"." +
                    ...
                    "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n" +
                    "\tat the top of the " + TopStarGionasMod.class.getSimpleName() + " java file.");
        }
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo)->{
            ...
            return initializers.contains(TopStarGionasMod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
    ...
    public void receiveEditCharacters() {
        TheCaretaker.Meta.registerCharacter();
    }
