...
public class CounterMeasures extends BaseCard {
    // makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    public static final String ID = makeID(CounterMeasures.class.getSimpleName());
    ...
    public void CardEffects(AbstractCreature Target, int TimesToDo) {
        int x = 0;
        while (x < TimesToDo) {
            addToBot(new GainBlockAction(Target, block));
            addToBot(new DrawCardAction(Target, 1));
