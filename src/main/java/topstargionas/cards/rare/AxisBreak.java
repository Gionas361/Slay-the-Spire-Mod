...
public class AxisBreak extends BaseCard {
    ...
    private static final int FOLLOWUPS = 3;
    private static final int UPG_FOLLOWUPS = 2;
    ...
    public AxisBreak() {
        ...
        setDamage(FOLLOWUPS, UPG_FOLLOWUPS);
        setCustomVar("EXPLO", 35, 22);
        setCustomVar("ENREQ", 10, -3);
    ...
    public void CardEffects(AbstractPlayer User, AbstractCreature Target, int TimesToDo) {
        int enOnUse = EnergyPanel.totalCount;
        int x = 0;
        while (x < TimesToDo) {
            int e = 0;
            while (e < enOnUse) {
                addToBot(new DamageAction(Target, new DamageInfo(User, damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.LIGHTNING));
                addToBot(new LoseEnergyAction(1));
                e++;
            }

            if (enOnUse >= customVar("ENREQ")) addToBot(new DamageAction(Target, new DamageInfo(User, customVar("EXPLO"), DamageInfo.DamageType.HP_LOSS), AbstractGameAction.AttackEffect.FIRE));
            x++;
        }
    }
